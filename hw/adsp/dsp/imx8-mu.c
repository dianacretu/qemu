/* Core DSP MU support for i.MX8 audio DSP.
 *
...
/*static uint32_t imx8_mu_read(uint32_t reg)
{
        ...
        struct adsp_io_info *info)
{
    mu_reset(info);
    adsp->mu = info;
    adsp->timer[0].timer = timer_new_ns(QEMU_CLOCK_VIRTUAL, &imx8_ext_timer_cb, info);
