...
uint64_t adsp_mu_read(void *opaque, hwaddr addr,
                        unsigned size);
void adsp_mu_write(void *opaque, hwaddr addr,
                     uint64_t val, unsigned size);

/* driver reads from the MU */
uint64_t adsp_mu_read(void *opaque, hwaddr addr,
        unsigned size)
{
...
const MemoryRegionOps adsp_imx8_host_mu_ops = {
    .read = adsp_mu_read,
    .write = adsp_mu_write,
